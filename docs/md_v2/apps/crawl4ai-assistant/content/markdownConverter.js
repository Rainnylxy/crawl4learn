class MarkdownConverter {
  ...
  async convert(elements, options = {}) {
    ...
    for (const element of elements) {
      const markdown = await this.convertElement(
        element,
        this.conversionContext,
      );
      if (markdown.trim()) {
  ...
  async convertList(element, type, context) {
    ...
    for (const child of element.children) {
      if (child.tagName === "LI") {
        const markdown = await this.convertListItem(child, {
          ...context,
          listType: type,
        });
        if (markdown) {
  ...
  async convertBlockquote(element, context) {
    const lines = (await this.processChildren(element, context))
      .trim()
      .split("\n");
  ...
  postProcessTextOnly(markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (numberMatch) {
        ...
        line.match(/\b(points?|by|ago|hide|comments?)\b/i) &&
        currentItem
      ) {
