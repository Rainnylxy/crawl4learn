...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!mainContent) {
    console.warn(
      "TOC Generator: Main content area '#terminal-mkdocs-main-content' not found.",
    );
    return;
  }
  ...
  if (!tocElement) {
    if (!mainGrid) {
      console.warn(
        "TOC Generator: Flex container '.terminal-mkdocs-main-grid' not found to append ToC.",
      );
      return;
    }
  ...
  if (headings.length === 0) {
    console.info(
      "TOC Generator: No headings found on this page. ToC not generated.",
    );
  ...
  headings.forEach((heading, index) => {
    ...
    if (!heading.id) {
      ...
      heading.id = `toc-heading-${index}-${heading.textContent
        .toLowerCase()
        .replace(/\s+/g, "-")
        .replace(/[^a-z0-9-]/g, "")}`;
    }
  ...
  const observerCallback = (entries) => {
      ...
      if (entry.isIntersecting) {
        ...
        if (
          ...
          entry.boundingClientRect.top <
            topmostVisibleHeading.boundingClientRect.top
        ) {
    ...
    if (topmostVisibleHeading) {
      const newActiveLink = tocElement.querySelector(
        `a[href="#${topmostVisibleHeading.id}"]`,
      );
