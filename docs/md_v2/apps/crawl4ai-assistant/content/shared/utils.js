...
function makeDraggable(element) {
  ...
  const titlebar = element.querySelector(
    ".c4ai-toolbar-titlebar, .c4ai-titlebar",
  );
  if (!titlebar) return;

  titlebar.addEventListener("mousedown", (e) => {
    // Don't drag if clicking on buttons
    if (e.target.classList.contains("c4ai-dot") || e.target.closest("button"))
      return;
...
function makeDraggableByHeader(element) {
  ...
  header.addEventListener("mousedown", (e) => {
    ...
    if (
      e.target.id === "c4ai-close-debugger" ||
      e.target.closest("#c4ai-close-debugger")
    )
      return;
...
function applySyntaxHighlighting(codeElement) {
  ...
  const highlightedLines = lines.map((line) => {
    ...
    highlightedLine = highlightedLine.replace(
      /(["'])([^"']*)\1/g,
      '<span class="c4ai-string">$1$2$1</span>',
    );
    ...
    keywords.forEach((keyword) => {
      ...
      highlightedLine = highlightedLine.replace(
        regex,
        '<span class="c4ai-keyword">$1</span>',
      );
    });
    ...
    highlightedLine = highlightedLine.replace(
      /\b([a-zA-Z_]\w*)\s*\(/g,
      '<span class="c4ai-function">$1</span>(',
    );
...
function applySyntaxHighlightingJS(codeElement) {
  ...
  const highlightedLines = lines.map((line) => {
    ...
    highlightedLine = highlightedLine.replace(
      /(\/\*.*?\*\/)/g,
      '<span class="c4ai-comment">$1</span>',
    );
