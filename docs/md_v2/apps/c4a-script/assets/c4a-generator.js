...
c4aGenerator.forBlock["c4a_if_exists"] = function (block, generator) {
  ...
  if (thenCode.includes("\n")) {
    ...
    return (
      lines
        .map((line) => `IF (EXISTS \`${selector}\`) THEN ${line}`)
        .join("\n") + "\n"
    );
  } else if (thenCode) {
    // Single line
...
c4aGenerator.forBlock["c4a_if_not_exists"] = function (block, generator) {
  ...
  if (thenCode.includes("\n")) {
    ...
    return (
      lines
        .map((line) => `IF (NOT EXISTS \`${selector}\`) THEN ${line}`)
        .join("\n") + "\n"
    );
  } else if (thenCode) {
...
c4aGenerator.forBlock["c4a_if_js"] = function (block, generator) {
  ...
  if (thenCode.includes("\n")) {
    const lines = thenCode.split("\n").filter((line) => line.trim());
    return (
      lines.map((line) => `IF (\`${condition}\`) THEN ${line}`).join("\n") +
      "\n"
    );
  } else if (thenCode) {
...
c4aGenerator.scrub_ = function (block, code, opt_thisOnly) {
  ...
  if (nextBlock) {
    if (!opt_thisOnly) {
      ...
      if (
        currentIsComment !== nextIsComment &&
        code.trim() &&
        nextCode.trim()
      ) {
        nextCode = "\n" + nextCode;
