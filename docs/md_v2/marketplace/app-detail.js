...
class AppDetailPage {
  ...
  async loadAppDetails() {
    try {
      ...
      try {
        ...
        this.appData = apps.find(
          (app) =>
            app.slug === this.appSlug ||
            app.name.toLowerCase().replace(/\s+/g, "-") === this.appSlug,
        );
        if (this.appData) {
          this.renderAppDetails();
        } else {
  ...
  renderAppDetails() {
    ...
    document.getElementById("app-type").textContent =
      this.appData.type || "Open Source";
    ...
    document.getElementById("app-downloads").textContent = this.formatNumber(
      this.appData.downloads || 0,
    );
    ...
    document.getElementById("app-contact") &&
      (document.getElementById("app-contact").textContent =
        this.appData.contact_email || "Not available");

    // Sidebar info
    document.getElementById("sidebar-downloads").textContent =
      this.formatNumber(this.appData.downloads || 0);
    document.getElementById("sidebar-rating").textContent = (
      this.appData.rating || 0
    ).toFixed(1);
    ...
    document.getElementById("sidebar-status").textContent =
      this.appData.status || "Active";
    document.getElementById("sidebar-pricing").textContent =
      this.appData.pricing || "Free";
    document.getElementById("sidebar-contact").textContent =
      this.appData.contact_email || "contact@example.com";
  ...
  renderTabContents() {
    ...
    if (overviewDiv) {
      if (this.appData.long_description) {
        overviewDiv.innerHTML = this.renderMarkdown(
    ...
    if (integrationDiv) {
      if (this.appData.integration_guide) {
        integrationDiv.innerHTML = this.renderMarkdown(
          this.appData.integration_guide,
        );
        // Add copy buttons to all code blocks
        this.addCopyButtonsToCodeBlocks(integrationDiv);
      } else {
        integrationDiv.innerHTML =
          "<p>Integration guide not yet available. Please check the official website for details.</p>";
      }
    }
  ...
  async loadRelatedApps() {
    try {
      const response = await fetch(
        `${API_BASE}/apps?category=${encodeURIComponent(this.appData.category)}&limit=4`,
      );
      const apps = await response.json();

      const relatedApps = apps
        .filter((app) => app.slug !== this.appSlug)
        .slice(0, 3);
      const grid = document.getElementById("related-apps-grid");

      grid.innerHTML = relatedApps
        .map(
          (app) => `
