...
A code snippet: \`crawler.run()\`. Check the [quickstart](/core/quickstart).`;
    ...
    if (message.sender === "ai") {
      ...
      messageDiv.querySelectorAll("pre code:not(.hljs)").forEach((block) => {
        if (typeof hljs !== "undefined") {
          ...
          console.warn(
            "highlight.js (hljs) not found for syntax highlighting.",
          );
        }
      });
      ...
      if (
        ...
        !message.text &&
        !messageDiv.querySelector(".thinking-indicator-cursor")
      ) {
  ...
  function streamSimulatedResponse(messageDiv, fullText) {
    const thinkingIndicator = messageDiv.querySelector(
  ...
  function updateCitationsDisplay() {
    ...
    if (!lastCitations) {
      citationsList.innerHTML =
        '<li class="no-citations">No citations available.</li>';
      return;
    }
      ...
      try {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            console.warn(
              "Ask AI: Could not clean parent URL using replaceState.",
              e,
            );
        ...
        try {
          ...
          console.warn(
            "Ask AI: Could not clean parent URL after decode error.",
            cleanError,
          );
        }
        return false;
  ...
  function loadChat(chatId) {
    ...
    if (storedChat === null) {
      console.warn(
        `Attempted to load non-existent chat: ${chatId}. Removing from index.`,
      );
  ...
  function renderHistoryList(indexArray) {
    ...
    if (!indexArray || indexArray.length === 0) {
      historyList.innerHTML =
        '<li class="no-history">No past chats found.</li>';
      return;
    }
  ...
  function setActiveHistoryItem(chatId) {
    ...
    const activeLi = document.querySelector(
      `#history-list li[data-chat-id="${chatId}"]`,
    );
    if (activeLi) {
  ...
  console.warn(
    "Virtual scrolling not implemented. Performance may degrade with very long chat histories.",
  );
});
