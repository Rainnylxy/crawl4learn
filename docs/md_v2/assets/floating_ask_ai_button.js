...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const baseUrl =
    window.location.origin + (currentPath.startsWith("/core/") ? "../.." : "");

  // Check if the current page IS the Ask AI page
  // Use includes() for flexibility (handles trailing slash or .html)
  if (currentPath.includes(askAiPagePath.replace(/\/$/, ""))) {
    // Remove trailing slash for includes check
    console.log(
      "Floating Ask AI Button: Not adding button on the Ask AI page itself.",
    );
