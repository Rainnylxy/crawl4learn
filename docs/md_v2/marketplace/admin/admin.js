...
const { API_BASE, API_ORIGIN } = (() => {
  ...
  let storedOverride = "";
  try {
    storedOverride = cleanOrigin(
      localStorage.getItem("marketplace_api_origin"),
    );
  } catch (error) {
    storedOverride = "";
  }
  ...
  if (origin) {
    ...
    return {
      API_BASE: `${normalized}/marketplace/api`,
...
class AdminDashboard {
  ...
  async loadStats() {
    ...
    document.getElementById("stat-sponsored").textContent =
      stats.apps.sponsored;
    document.getElementById("stat-articles").textContent = stats.articles;
    document.getElementById("stat-sponsors").textContent =
      stats.sponsors.active;
    document.getElementById("stat-views").textContent = this.formatNumber(
      stats.total_views,
    );
  }
  ...
  async loadArticles() {
    this.data.articles = await this.apiCall(
      `/articles?limit=100&_=${Date.now()}`,
      {
        cache: "no-store",
      },
    );
  ...
  async loadSponsors() {
    const cacheBuster = Date.now();
    this.data.sponsors = await this.apiCall(
      `/sponsors?limit=100&_=${cacheBuster}`,
      {
        cache: "no-store",
      },
    );
  ...
  getSponsorForm(sponsor) {
    const existingFile = sponsor?.logo_url
      ? sponsor.logo_url.split("/").pop().split("?")[0]
      : "";
    return `
  ...
  collectFormData(type) {
    ...
    if (type === "apps") {
      ...
      const downloads = parseInt(
        document.getElementById("form-downloads").value,
        10,
      );
      ...
      data.sponsored = document.getElementById("form-sponsored").checked
        ? 1
        : 0;
      data.long_description = document.getElementById(
        "form-long-description",
      ).value;
      data.integration_guide =
        document.getElementById("form-integration").value;
  ...
  async deleteCategory(id) {
    const hasApps = this.data.apps.some(
      (app) =>
