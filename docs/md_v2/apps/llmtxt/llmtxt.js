...
document.addEventListener("DOMContentLoaded", () => {
  ...
  state.selectedContextTypes.set(
    firstComponent.id,
    new Set(["memory", "reasoning"]),
  );
  updateComponentUI();
});
...
function updateComponentUI() {
  components.forEach((component) => {
    ...
    const selectedTypes =
      state.selectedContextTypes.get(component.id) || new Set();
...
async function fetchTokenCount(componentId, type) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Update UI
      const tokenSpan = document.getElementById(
        `tokens-${componentId}-${type}`,
      );
      if (tokenSpan) {
...
async function handleDownload() {
  ...
  try {
    ...
    if (files.length === 0) {
      throw new Error(
        "No files selected. Please select at least one component or preset.",
      );
    }
  ...
  const promises = fileInfos.map(async (fileInfo) => {
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          `Failed to fetch ${fileInfo.fileName} from ${fileInfo.baseUrl + fileInfo.fileName}`,
        );
        return {
          fileInfo,
          content: `<!-- Failed to load ${fileInfo.fileName} -->`,
        };
      }
      const content = await response.text();
      ...
      return {
        fileInfo,
        content: `<!-- Error loading ${fileInfo.fileName} -->`,
      };
    }
  });
...
function renderReferenceTable() {
  ...
  components.forEach((component) => {
    ...
    const reasoningTokens =
      state.tokenCounts.get(`${component.id}-reasoning`) || 0;
    const examplesTokens =
      state.tokenCounts.get(`${component.id}-examples`) || 0;
