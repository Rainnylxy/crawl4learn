...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!headerContainer) {
    console.warn(
      "GitHub Stats: Header container not found with selector:",
      targetHeaderSelector,
    );
    return;
  }

  const repoLinkElement = headerContainer.querySelector(
    'a[href*="github.com/"]',
  ); // Find the existing GitHub link
  ...
  if (!owner || !repo) {
    console.warn(
      "GitHub Stats: Could not extract owner/repo from URL:",
      repoUrl,
    );
    return;
  }
  ...
  let version = "";
  const siteTitleElement = headerContainer.querySelector(
    ".terminal-title, .site-title",
  ); // Adjust selector based on theme's title element
  ...
  if (!version) {
    console.info(
      "GitHub Stats: Could not extract version from title. You might need to adjust the selector or regex.",
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(
        `GitHub Stats: API request failed with status ${response.status}. Rate limit exceeded?`,
      );
      stars = "N/A";
      forks = "N/A";
    }
  ...
  const insertBeforeElement = insertBeforeSelector
    ? headerContainer.querySelector(insertBeforeSelector)
    : null;
  if (insertBeforeElement) {
    // headerContainer.insertBefore(badgeContainer, insertBeforeElement);
    headerContainer
      .querySelector(insertBeforeSelector)
