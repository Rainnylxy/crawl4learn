...
class TutorialApp {
  ...
  setupButtons() {
    ...
    document
      .getElementById("skip-tutorial-btn")
      .addEventListener("click", () => {
        this.hideIntroModal();
      });
    ...
    this.tutorialSteps = [
      {
        ...
        description:
          "C4A-Script is a simple language for web automation. Let's start by handling popups that appear on websites.",
        script:
      ...
      },
      {
        ...
        description:
          "Now let's handle the newsletter popup that appears after 3 seconds.",
        script:
          "# Wait for newsletter popup to appear\nWAIT 3\n\n# Close the newsletter popup\nIF (EXISTS `#newsletter-popup`) THEN CLICK `.close`",
        validate: () => {
          const iframe = document.getElementById("playground-frame");
      ...
      },
      {
        ...
        description:
          "Click the start button to reveal more interactive elements.",
      ...
      },
      {
        title: "Login Process",
        description:
          "Now let's complete the login form with email and password using the new SET command.",
        script:
          '# Login process\nCLICK `#login-btn`\nWAIT `.login-form` 2\n\n# Fill form fields using SET\nSET `#email` "demo@example.com"\nSET `#password` "demo123"\n\n# Submit the form\nCLICK `button[type="submit"]`\nWAIT `.success` 2',
        validate: () => {
          const iframe = document.getElementById("playground-frame");
          return (
            iframe.contentDocument?.querySelector(".user-info")?.style
              .display === "flex"
          );
        },
      },
      {
        ...
        description:
          "Navigate to the catalog and use scrolling to load more products.",
        script:
  ...
  showStep(index) {
    ...
    document.getElementById("tutorial-step-info").textContent =
      `Step ${index + 1} of ${this.tutorialSteps.length}`;
    ...
    try {
      ...
      if (compiled.success) {
        ...
        this.addConsoleMessage(
          `✓ Compiled successfully (${compiled.jsCode.length} statements)`,
          "success",
        );
        ...
        this.addConsoleMessage(
          `✗ Compilation error at line ${error.line}:${error.column}`,
          "error",
        );
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          column: 1,
          message:
            "C4A compilation service unavailable. Please ensure the server is running.",
  ...
  async executeInIframe(jsCode) {
    ...
    return new Promise((resolve, reject) => {
      const messageHandler = (event) => {
        if (
          event.data.type === "c4a-execution-complete" &&
          event.data.id === executionId
        ) {
  ...
  toggleJSEdit() {
    ...
    if (this.isEditingJS) {
      ...
      this.addConsoleMessage(
        "JS edit mode enabled - modify and run",
        "warning",
      );
    } else {
  ...
  showExamples() {
    ...
    localStorage.setItem(
      "exampleIndex",
      ((currentIndex + 1) % examples.length).toString(),
    );
  }
...
class RecordingManager {
  ...
  setupUI() {
    ...
    document
      ...
      ?.addEventListener("click", () => this.hideTimeline());
    ...
    document
      .getElementById("generate-script")
      ?.addEventListener("click", () => this.generateScript());
  }
  ...
  removeEventCapture() {
    ...
    iframe.contentWindow.eval(
      "if (window.__c4aCleanupRecording) window.__c4aCleanupRecording();",
    );
    this.eventInjected = false;
  }
  ...
  flushKeyBuffer() {
      ...
      .map((e) => {
        switch (e.key) {
          ...
          case "Tab":
            return "\t";
          case "Backspace":
            return ""; // Skip backspace in final text
          case "Delete":
            return ""; // Skip delete in final text
          default:
            return e.key;
        }
      })
    ...
    const existingCommand = this.groupedEvents.find(
      (cmd) =>
    ...
    switch (event.type) {
      case "click":
        ...
        if (event.targetTag?.toLowerCase() === "select") {
          ...
          for (
            let i = index + 1;
            i < Math.min(index + 5, this.rawEvents.length);
            i++
          ) {
            if (
              this.rawEvents[i].type === "change" &&
      ...
      case "wheel":
        ...
        if (event.deltaY !== 0) {
          ...
          if (
            this.scrollAccumulator.direction === direction &&
  ...
  setupEditModalHandlers() {
    ...
    const saveHandler = () => {
      ...
      switch (event.type) {
        ...
        case "SCROLL":
          ...
          event.amount =
            parseInt(document.getElementById("edit-value").value) || 0;
          break;
        case "WAIT":
          event.value =
            parseInt(document.getElementById("edit-value").value) || 0;
    ...
    document
      .getElementById("event-editor-overlay")
      .addEventListener("click", closeModal);
  }
  ...
  generateScript() {
    ...
    const script = selectedEvents
      .map((event) => this.eventToC4A(event))
      .join("\n");

    // Set the script in the editor
    this.app.editor.setValue(script);
    this.app.addConsoleMessage(
      `Generated ${selectedEvents.length} commands`,
      "success",
    );
