...
document.getElementById("extract-btn").addEventListener("click", async (e) => {
  ...
  if (!model_name) {
    showToast(
      "Please select a model from the dropdown or add one from the Models page",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    const endpoint =
      scraping_approach === "llm" ? "/scrape-with-llm" : "/scrape";
    ...
    if (response.ok) {
      ...
      showToast(
        `Data extracted successfully using ${scraping_approach === "llm" ? "LLM-based" : "Schema-based"} approach!`,
        "success",
      );
    } else {
...
function displayModels(models) {
  if (models.length === 0) {
    modelsList.innerHTML =
      '<p style="text-align: center; color: #7f8c8d; padding: 2rem;">No models saved yet. Add your first model above!</p>';
    return;
  }
...
function displayResults(result) {
  ...
  document.getElementById("result-model").textContent =
    result.model_name || "Default Model";
...
function displaySavedRequests(requests) {
  ...
  if (requests.length === 0) {
    requestsList.innerHTML =
      '<p style="text-align: center; color: #CCCCCC; padding: 2rem;">No saved API requests yet. Make your first request from the Scrape page!</p>';
    return;
  }
...
async function deleteSavedRequest(requestId) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/saved-requests/${requestId}`,
      {
        method: "DELETE",
      },
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(`${API_BASE_URL}/health`)
    ...
    .catch(() => {
      showToast(
        "Warning: Cannot connect to API server. Make sure it's running on localhost:8000",
        "error",
      );
    });
});
